...
def sync_freshdesk(db: Session, days: int = 30) -> dict[str, Any]:
    ...
    try:
        ...
        for business_date, values in grouped_daily.items():
            row = db.execute(select(FreshdeskTicketsDaily).where(FreshdeskTicketsDaily.business_date == business_date)).scalars().first()
            if row is None:
                row = FreshdeskTicketsDaily(business_date=business_date)
                db.add(row)
            ...
            row.first_response_hours = (
                values["first_response_hours_total"] / values["first_response_count"]
                if values["first_response_count"] else 0.0
            )
            row.resolution_hours = (
                values["resolution_hours_total"] / values["resolution_count"]
                if values["resolution_count"] else 0.0
            )
            row.sla_breach_rate = (
                values["sla_breaches"] / values["tickets_created"] * 100.0
                if values["tickets_created"] else 0.0
            )
            row.csat = (
                values["csat_total"] / values["csat_count"]
                if values["csat_count"] else 0.0
            )

        for (business_date, agent_id), values in grouped_agent.items():
            row = db.execute(
                select(FreshdeskAgentDaily).where(
                    FreshdeskAgentDaily.business_date == business_date,
                    FreshdeskAgentDaily.agent_id == agent_id,
                )
            ).scalars().first()
            if row is None:
        ...
        for (business_date, group_name), values in grouped_group.items():
            row = db.execute(
                select(FreshdeskGroupsDaily).where(
                    FreshdeskGroupsDaily.business_date == business_date,
                    FreshdeskGroupsDaily.group_name == group_name,
                )
            ).scalars().first()
            if row is None:
