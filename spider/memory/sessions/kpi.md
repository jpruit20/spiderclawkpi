...
- 2026-04-06 GA4 hardening pass: backend now treats GA4 as requiring full Google service-account JSON fields (`GA4_CLIENT_EMAIL`, `GA4_PRIVATE_KEY`, `GA4_PROJECT_ID`, `GA4_PROPERTY_ID`), validates them explicitly, fails fast at startup when they are malformed, and exposes an authenticated GA4 debug self-check endpoint that reports masked identity/project/property plus token/runReport success without leaking secrets.
