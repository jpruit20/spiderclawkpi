...
- 2026-04-06 GA4 follow-up: production GA4 was failing before token issuance because the private key in `.env` is stored as a single-line string with literal `\n` escapes. Patched the GA4 connector to normalize escaped newlines/quotes before JWT signing. Also updated the frontend live-connector registry so Executive source-trust counts include `ga4` and `clarity` alongside Shopify, Triple Whale, and Freshdesk.
